...
export default function SidebarMahasiswa({
  ...
  return (
    <div
      className={`fixed top-0 left-0 h-full w-80 bg-black text-white z-40 transition-transform duration-300 ease-in-out ${
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="relative overflow-hidden flex-1">
          <div className="relative w-full h-full">
            ...
            >
              <nav className="flex flex-col divide-y divide-gray-800">
                <button
                  ...
                  className="flex justify-between items-center w-full text-left px-4 py-3 text-sm hover:bg-gray-700"
                >
                  HOME
                </button>
                <button
                ...
                {/* Profile menu opens submenu */}
                <button
                  onClick={() => handleOpenSubmenu("profile")}
                  className="flex justify-between items-center w-full text-left px-4 py-3 text-sm hover:bg-gray-700"
                >
                  <span>{userData?.nim || "NIM Tidak Ditemukan"}</span>
            ...
            <div
              className={`absolute top-0 left-0 w-full h-full transition-transform duration-300 ease-in-out ${
                activeMenu === "faculty" ? "translate-x-0" : "translate-x-full"
              }`}
            >
              <nav className="flex flex-col divide-y divide-gray-800">
            ...
            <div
              className={`absolute top-0 left-0 w-full h-full transition-transform duration-300 ease-in-out ${
                activeMenu === "helpdesk" ? "translate-x-0" : "translate-x-full"
              }`}
            >
              <nav className="flex flex-col divide-y divide-gray-800">
            ...
            <div
              className={`absolute top-0 left-0 w-full h-full transition-transform duration-300 ease-in-out ${
                activeMenu === "profile" ? "translate-x-0" : "translate-x-full"
              }`}
            >
              <nav className="flex flex-col divide-y divide-gray-800">
                <button onClick={() => navigate("/mahasiswa/profile")} className="px-4 py-3 text-sm text-left hover:bg-gray-700">My Profile</button>
                <button onClick={() => navigate("/mahasiswa/grades")} className="px-4 py-3 text-sm text-left hover:bg-gray-700">Grades</button>
                <button onClick={() => navigate("/mahasiswa/messages")} className="px-4 py-3 text-sm text-left hover:bg-gray-700">Messages</button>
                <button onClick={() => navigate("/mahasiswa/preferences")} className="px-4 py-3 text-sm text-left hover:bg-gray-700">Preferences</button>
                <button onClick={handleLogout} className="px-4 py-3 text-sm text-left hover:bg-gray-700 text-red-400">Log Out</button>
              </nav>
            </div>
          </div>
        </div>
